...
from torch_geometric.nn import (GATConv,
                                ...
                                max_pool_neighbor_x,
                                global_add_pool)
from src.models.layers import CoAttentionLayer, RESCAL


class MI_DDI(nn.Module):
    def __init__(self, in_features, hidd_dim, kge_dim, rel_total, heads_out_feat_params, blocks_params):
        ...
        for i, (head_out_feats, n_heads) in enumerate(zip(heads_out_feat_params, blocks_params)):
            block = MI_DDI_Block(n_heads, in_features, head_out_feats, final_out_feats=self.hidd_dim)
    ...
    def forward(self, triples):
        ...
        # attentions = None ekleme yapıldı
        scores = self.KGE(kge_heads, kge_tails, rels, attentions)

        return scores,attentions


class MI_DDI_Block(nn.Module):
